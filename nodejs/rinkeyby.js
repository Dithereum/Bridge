...
async function company_bridge_send_method(_toWallet, _amt, _chainid, orderid){
    ...
    if(MY_PROVIDER){
		    ...
		    try{
		      ...
		      //var requiredGas = await company_bridgeinstance.methods.tokenOut( _toWallet.toString(), _toWallet.toString(), _amt.toString(), orderid.toString(), _chainid.toString()).estimateGas({from: bridge_admin_wallet.toString()});
	 ...
	 try{
	    await myinstance.getPastEvents('TokenIn', {        
	        ...
	        //events.forEach((result)=>{	        		
	        	   console.log("ZZZZ>>>>",result.returnValues);
					console.log("ZZZZ>>>>orderID: >>>",result.returnValues.orderID);
					console.log("ZZZZ>>>>tokenAddress: >>>",result.returnValues.tokenAddress);
					console.log("ZZZZ>>>>user: >>>",result.returnValues.user);
					console.log("ZZZZ>>>>value: >>>",result.returnValues.value);
					console.log("ZZZZ>>>>ChainID: >>>",result.returnValues.chainID);
					var _orderid = result.returnValues.orderID;
					var _chainid = result.returnValues.chainID;
					var _sendcoinsTo = result.returnValues.user.toString();
					var _amount = result.returnValues.value.toString();

					if(_chainid && parseInt(_amount)){
						console.log(_chainid, _sendcoinsTo, _amount);						
						//(async ()=>{
							try{
...
async function checkLatestBlock(){
    console.log("Setting variable ...");    
 	 process.env.lastLatestBlockNumber = await web3.eth.getBlockNumber() ? await web3.eth.getBlockNumber() : 9668725;      
